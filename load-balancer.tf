...
resource "azurerm_network_interface_backend_address_pool_association" "ba_association" {
    count                   = var.arm_frontend_instances
    network_interface_id    = "${element(azurerm_network_interface.frontend.*.id, count.index)}"
    ip_configuration_name   = "${element(azurerm_network_interface.frontend.*.ip_configuration.0.name, count.index)}"
